/*! html5-player - v1.3.0 - 2014-09-01 */
"use strict";
...
(function($) {
    ...
    window.HtmlContentPlayer = function(args) {
        ...
        function _timeUpdated(event) {
            ...
            if (_options.audio) {
                ...
                if (_keyframeElapsed > 40) {
                    if (_isPlaying) {
                        playFromTime(currentTime * 1e3);
                        if (_options.triggerOnSync) trigger("played", null, currentTime * 1e3);
                    } else {
                        stopAtTime(currentTime * 1e3);
                        if (_options.triggerOnSync) trigger("paused", null, currentTime * 1e3);
                    }
                    _keyframeElapsed = 0;
                }
        ...
        function gotoTimeAsPercentage(percentage) {
            ...
            if (_isPlaying) {
                playFromTime(_options.duration * percentage * 1e3);
                trigger("played", null, _options.duration * percentage * 1e3);
            } else {
                stopAtTime(_options.duration * percentage * 1e3);
                trigger("paused", null, _options.duration * percentage * 1e3);
            }
            if (_options.audio) {
                if (_useAudioKludge && _audioTrack.duration && !isNaN(_audioTrack.duration)) {
                    _timelineDifference = _audioTrack.currentTime * 1e3 - _stage.getPosition();
                }
            }
        }
        function play(time) {
        ...
        function stopAllChildSymbols(symbol, time) {
            ...
            for (var i = 0; i < childSymbols.length; i++) {
                stopAllChildSymbols(childSymbols[i], time);
            }
            return;
        }
        ...
        function getCurrentTime() {
            if (_options.audio) {
                return (_audioTrack.currentTime * 1e3 - _timelineDifference) / 1e3;
            } else {
                return _stage.getPosition() / 1e3;
            }
        }
